...
==> The positional arguments are those where position matters: first goes to first, second goes to second, and so on.  

2) Keyword argument  
==> In case of keyword arguments, we pass the parameter name, so position doesn’t matter.  

3) Default argument  
==> Default argument means we give a default value; even if you don’t pass an argument, it will not show any error.  

4) Positional arbitrary argument  
==> Positional arbitrary argument allows multiple values as a tuple. We use it when we don’t know how many arguments will come.  

5) Keyword arbitrary argument  
==> Keyword arbitrary argument allows multiple values as a dictionary. We use it when we don’t know how many arguments will come.  
